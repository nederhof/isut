...
const allStructure = /([A-I]|[K-Z]|(?:Aa)|(?:NL)|(?:NU))([1-9](?:[0-9][0-9]?)?)([a-z]?)|[a-zA-Z]+|:|\*|\^st|\^sb|\^et|\^sb|\+|\(|\)/g;
...
class UniHiero {
	...
	textToName(text) {
		const matches = text.match(allStructure);
	...
	static cmpTexts(names1, names2) {
		if (names1.length == 0) {
			...
			if (names2.length == 0)
				return 1;
			const cmp = UniHiero.cmpText(names1[0], names2[0]);
			if (cmp != 0)
				return cmp;
			else 
				return UniHiero.cmpTexts(names1.slice(1), names2.slice(1));
		}
	}
}
