...
from graphics2 import add_background
from reduction2 import get_reduction
...
def get_images(pref):
	vecs = []
	for file_name in sorted(os.listdir(HANDDRAWN_DIR)):
		if file_name.startswith(pref):
			path = os.path.join(HANDDRAWN_DIR, file_name)
			im = Image.open(path)
